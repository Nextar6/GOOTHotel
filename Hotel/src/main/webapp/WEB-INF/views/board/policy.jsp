...
<%@ include file="header.jspf"%>
	<br>
<h1 style="float: left; color: white;">약관 및 정책</h1>
...
<table class="type">
		...
		<tbody>
			<c:forEach var="vo" items="${policyList }">
				<tr>
					<td>${vo.policyNo }</td>
					<td><a
										href="policy-detail?policyNo=${vo.policyNo}&page=${pageMaker.criteria.page}">${vo.policyTitle }</a></td>
					<td>${vo.writer }</td>
					<fmt:formatDate value="${vo.policyCdate }"
										pattern="yyyy-MM-dd HH:mm:ss" var="policyCdate" />
					<td>${policyCdate }</td>
				</tr>
